...
from Predict.Predict import pre
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def pbtPredict_Callback(self):
        ...
        img_array = np.array(pil_img.convert('L')).reshape(1,28, 28, 1) / 255.0
        ...
        result_class = model.predict(img_array)
        softmax = model.call(img_array)
        self.result[0] = np.argmax(result_class)  # 预测的数字
        self.result[1] = np.max(softmax)  # 置信度
    ...
    def pbtGetMnist_Callback(self):
        ...
        data = MNISTLoader()
        # 随机抽取一张测试集图片，放大后显示
        img,y = data.get_batch(1)  # shape:[1,28,28]
        print(img.shape)
